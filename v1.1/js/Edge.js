...
function Edge(model, config) {
  ...
  self.updateSignals = function () {
    ...
    while (lastSignal && lastSignal.position >= 1) {
      ...
      lastSignal.delta = self.strength * 0.33; // flip at the end only!
      console.log("delta: ", lastSignal.delta);
  ...
  self.isPointOnLabel = function (x, y) {
    ...
    var radius;
    if (self.loopy.tool == Loopy.TOOL_DRAG || self.loopy.tool == Loopy.TOOL_INK)
      radius = 40; // selecting, wide radius!
    else if (self.loopy.tool == Loopy.TOOL_ERASE)
      radius = 25; // no accidental erase
